;(function ($) {
    ...
    $(document).ready(function () {
       ...
       /* $('.global_setting_group').on('click', '.checkbox', function () {
        ...
        $("#cbcurrencyconverter_resetinfo_wrap").on(
            "click",
            ".cbcurrencyconverter_setting_options_check_action_call",
            function (e) {
                e.preventDefault();

                var $this = $(this);
                $("#cbcurrencyconverter_resetinfo_wrap").find(":checkbox").prop("checked", true);
            }
        );

        $("#cbcurrencyconverter_resetinfo_wrap").on(
            "click",
            ".cbcurrencyconverter_setting_options_check_action_ucall",
            function (e) {
                e.preventDefault();

                var $this = $(this);
                $("#cbcurrencyconverter_resetinfo_wrap").find(":checkbox").prop("checked", false);
            }
        );
                ...
                $.ajax({
                    ...
                    success: function (data, textStatus, XMLHttpRequest) {
                       //window.location.href = data.url;
                        if(data.success){
                            new AWN(cbcurrencyconverter_awn_options).success(data.message);
                            window.location.href = data.url;
                        }
                        else{
                            new AWN(cbcurrencyconverter_awn_options).alert(data.message);
                        }
        ...
        $('#reset_transient_trigger').on('click', function (e) {
            ...
            var onOk = () => {
                ...
                $.ajax({
                    ...
                    url: cbcurrencyconverter_setting.ajaxurl,
                    data: {
                        action: 'cbcurrencyconverter_transient_reset',
                        security: cbcurrencyconverter_setting.nonce,
                    },
                    success: function (data, textStatus, XMLHttpRequest) {
                        //window.location.href = data.url;
